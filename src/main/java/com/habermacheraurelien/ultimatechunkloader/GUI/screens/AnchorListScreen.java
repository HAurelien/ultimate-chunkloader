...
public class AnchorListScreen extends ModUpdatableScreen {
    ...
    protected void init() {
        ...
        List<ChunkAnchorBlockModel> list = anchors.getChunkAnchorBlockModelList();
        ...
        // 🔧 Add filler rows if not enough elements to visually fill page
        int actualDisplayed = endIndex - startIndex;
        for (int i = actualDisplayed; i < itemsPerPage; i++) {
            SpacerElement spacer = new SpacerElement(0, rowHeight - 5); // Slight reduction to avoid overshoot from spacing
    ...
    private void nextPage() {
        int totalPages = (int) Math.ceil((double) anchors.getChunkAnchorBlockModelList().size() / itemsPerPage);
